...
class Quiz:
    ...
    def __init__(self, how_many):
        ...
        # Initialize choice button references
        self.choice_button_ref = []  # List to track answer buttons
    ...
    def get_round_animals(self):
        ...
        while len(round_animal_list) < 2:
            ...
            if adult_name not in used_animals:
                ...
                # Add both names to the round_animal_list
                round_animal_list.append((adult_name, youth_name))
    ...
    def new_round(self):
        ...
        if current_round >= how_many:
            self.round_results_label.config(
                text="Quiz Complete! Your score: {}/{}".format(self.rounds_won.get(), how_many))
            self.next_button.config(state=DISABLED)
            return

        # disable next button (enable it at the end of the round)
        self.next_button.config(state=DISABLED)

        # Clear previous button references
        self.button_animals_list.clear()

        # Get new animals for buttons
        self.button_animals_list = self.get_round_animals()

        # Select a random question for the new round.
        current_question = random.choice(self.button_animals_list)
        ...
        self.animal_label.config(text=f"What is the youth of {self.adult_name}?")
        self.intro_heading_label.config(text=f"Round {self.rounds_played.get() + 1} of {self.rounds_wanted.get()}")

        # Creating answer buttons
        self.choice_button_ref.clear()
        for idx, (adult, youth) in enumerate(self.button_animals_list):
            self.choice_button = Button(self.choice_frame,
                                        text=youth,  # Displaying youth name
                                        width=15,
                                        command=lambda ans=youth: self.to_compare(ans))
            self.choice_button.grid(row=idx // 2, column=idx % 2, padx=5, pady=5)
            self.choice_button_ref.append(self.choice_button)

        # Add additional wrong answers to the button list
        while len(self.choice_button_ref) < 4:
            wrong_animal = random.choice(self.all_animals)
            wrong_youth = wrong_animal[1]

            # Avoid duplicates
            if wrong_youth not in [button['text'] for button in self.choice_button_ref]:
                self.choice_button = Button(self.choice_frame,
                                            text=wrong_youth,
                                            width=15,
                                            command=lambda ans=wrong_youth: self.to_compare(ans))
                self.choice_button.grid(row=len(self.choice_button_ref) // 2, column=len(self.choice_button_ref) % 2,
                                        padx=5, pady=5)
                self.choice_button_ref.append(self.choice_button)
    ...
    def to_compare(self, user_score):
        ...
        if user_score == self.correct_answer:  # Use self.correct_answer
            self.round_results_label.config(text="Correct!", bg="#C6EFCE")
            self.rounds_won.set(self.rounds_won.get() + 1)
        else:
            self.round_results_label.config(text="Wrong!", bg="#FFC7CE")
